import { Client, Databases, ID, Query, type Models, IndexType } from 'node-appwrite';
...
// Collection configurations (Original Version)
// const COLLECTIONS = {
//   TAX_RATES: 'tax_rates' ,
//   ORDERS: 'orders',
...
const collectionAttributes: Record<string, any> = {
  [COLLECTIONS.TAX_RATES || 'default']: [
    ...
    { key: 'appliesToShipping', type: 'boolean', required: false, default: false },
  ...
  [COLLECTIONS.ORDERS || 'default']: [
    ...
    { key: 'subtotal', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'taxAmount', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'discountTotal', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'shippingAmount', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'finalPrice', type: 'double', required: true, min: 0.0, max: 1000000.0 },
  ...
  [COLLECTIONS.ORDER_ITEMS || 'default']: [
    ...
    { key: 'unitPrice', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'total', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'discountAmount', type: 'double', required: true, min: 0.0, max: 1000000.0 },
  ],
  [COLLECTIONS.DISCOUNTS || 'default']: [
    ...
    { key: 'minOrderAmount', type: 'double', required: false, min: 0.0, max: 1000000.0 },
    { key: 'maxDiscountAmount', type: 'double', required: false, min: 0.0, max: 1000000.0 },
    { key: 'startDate', type: 'string', size: 100, required: true },
    { key: 'endDate', type: 'string', size: 100, required: true },
    { key: 'isActive', type: 'boolean', required: false, default: true },
    { key: 'usageLimit', type: 'integer', required: false, min: 1, max: 1000000 },
    { key: 'usageCount', type: 'integer', required: false, default: 0, min: 0, max: 1000000 },
  ...
  [COLLECTIONS.DISCOUNT_USAGE || 'default']: [
    ...
    { key: 'userId', type: 'string', size: 36, required: true },
    { key: 'orderId', type: 'string', size: 36, required: true },
    { key: 'orderTotal', type: 'double', required: true, min: 0.0, max: 1000000.0 },
    { key: 'discountAmount', type: 'double', required: true, min: 0.0, max: 1000000.0 },
...
const collectionIndexes: Record<string, any[]> = {
  ...
  [COLLECTIONS.USER_DISCOUNTS || 'default']: [
    { key: 'idx_userId', type: 'key', attributes: ['userId'] },
    { key: 'idx_discountId', type: 'key', attributes: ['discountId'] },
    { key: 'idx_user_discount', type: 'unique', attributes: ['userId', 'discountId'] },
  ],
};
...
async function dropCollection(collectionId: string) {
  try {
    ...
    if (error.code !== 404) { // Ignore 404 errors (collection doesn't exist)
      throw error;
    }
  }
}
...
async function waitForAttribute(collectionId: string, attributeKey: string, maxRetries = 10) {
      ...
      try {
        ...
        if (error.code === 404) {
          ...
          const collection = await databases.createCollection(
            DATABASE_ID,
            collectionId,
            collectionName.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
          );
          ...
          for (const attr of attributes) {
            try {
              if (attr.type === 'integer') {
                if (typeof attr.min === 'number' && typeof attr.max === 'number') {
              ...
              console.log(`  - Added attribute: ${attr.key} (${attr.type}${attr.required ? ', required' : ''}${attr.min !== undefined ? `, min: ${attr.min}` : ''}${attr.max !== undefined ? `, max: ${attr.max}` : ''}${attr.default !== undefined ? `, default: ${attr.default}` : ''})`);

              // Add a small delay between attribute creation
              await new Promise(resolve => setTimeout(resolve, 300));
            } catch (error: any) {
              if (error.code !== 409) { // Ignore duplicate attribute errors
          ...
          const indexes = collectionIndexes[collectionId as keyof typeof collectionIndexes] || [];
          for (const index of indexes) {
            try {
              ...
              if (attributesReady.every(ready => ready)) {
                ...
                const indexType = index.type === 'unique' ? IndexType.Unique : IndexType.Key;
                ...
                console.log(`  - Skipping index ${index.key}: required attributes not ready`);
              }
            } catch (error: any) {
              console.error(`  - Failed to add index ${index.key}:`, error.message);
            }
          }
        } else {
